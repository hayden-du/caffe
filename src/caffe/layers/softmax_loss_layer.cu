...
__global__ void SoftmaxLossForwardGPU(const int nthreads,
  ...
  for( int idx = blockIdx.x*blockDim.x + threadIdx.x ; idx < nthreads ; idx += blockDim.x*gridDim.x ) {
    ...
    if (has_ignore_label_ && label_value == ignore_label_) {
      ...
      Mtype tmp = -log(max(prob_data[n * dim + label_value * spatial_dim + s], minDtype<Dtype>()));
      loss[idx] = tmp;
      counts[idx] = 1;
      lossSum += tmp;
      count += 1;
...
void SoftmaxWithLossLayer<Dtype,Mtype>::Forward_gpu(
  ...
  const Dtype* label = bottom[1]->gpu_data_base<Dtype>();
  const int dim = prob_.count() / outer_num_;
  const int nthreads = outer_num_ * inner_num_;
  // Since this memory is not used for anything until it is overwritten
  // on the backward pass, we use it here to avoid having to allocate new GPU
  // memory to accumulate intermediate results in the kernel.
  Dtype* loss_data = bottom[0]->mutable_gpu_diff_base<Dtype>();
...
template <typename Dtype, typename Mtype>
void SoftmaxWithLossLayer<Dtype,Mtype>::Backward_gpu(const vector<BlobBase*>& top,
