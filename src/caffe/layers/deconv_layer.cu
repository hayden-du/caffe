...
void DeconvolutionLayer<Dtype,Mtype>::Forward_gpu(const vector<BlobBase*>& bottom,
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    ...
    for (int n = 0; n < this->num_; ++n) {
      ...
      if (this->bias_term_) {
        const Dtype* bias = this->blobs_[1]->template gpu_data_base<Dtype>();
...
template <typename Dtype, typename Mtype>
void DeconvolutionLayer<Dtype,Mtype>::Backward_gpu(const vector<BlobBase*>& top,
      const vector<bool>& propagate_down, const vector<BlobBase*>& bottom) {
  const Dtype* weight = this->blobs_[0]->template gpu_data_base<Dtype>();
  Dtype* weight_diff = this->blobs_[0]->template mutable_gpu_diff_base<Dtype>();
  for (int i = 0; i < top.size(); ++i) {
    ...
    if (this->bias_term_ && this->param_propagate_down_[1]) {
      Dtype* bias_diff = this->blobs_[1]->template mutable_gpu_diff_base<Dtype>();
      for (int n = 0; n < this->num_; ++n) {
