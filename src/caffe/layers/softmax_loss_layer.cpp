...
void SoftmaxWithLossLayer<Dtype,Mtype>::LayerSetUp(
  ...
  softmax_layer_.reset(new SoftmaxLayer<Dtype,Mtype>(softmax_param));
...
void SoftmaxWithLossLayer<Dtype,Mtype>::Forward_cpu(
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      loss -= log(std::max(prob_data[i * dim + label_value * inner_num_ + j], minDtype<Dtype>()));
      ++count;
    }
  }
  if (normalize_) {
    top[0]->mutable_cpu_data_base<Dtype>()[0] = loss / count;
  } else {
    top[0]->mutable_cpu_data_base<Dtype>()[0] = loss / outer_num_;
  }
  if (top.size() == 2) {
...
template <typename Dtype, typename Mtype>
void SoftmaxWithLossLayer<Dtype,Mtype>::Backward_cpu(const vector<BlobBase*>& top,
