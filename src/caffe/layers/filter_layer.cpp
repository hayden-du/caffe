...
void FilterLayer<Dtype,Mtype>::LayerSetUp(const vector<BlobBase*>& bottom,
...
template <typename Dtype, typename Mtype>
void FilterLayer<Dtype,Mtype>::Reshape(const vector<BlobBase*>& bottom,
      const vector<BlobBase*>& top) {
  ...
  for (int item_id = 0; item_id < bottom[selector_index]->shape(0); ++item_id) {
    ...
    if (bottom_data_selector[item_id] != 0) {
...
template <typename Dtype, typename Mtype>
void FilterLayer<Dtype,Mtype>::Forward_cpu(const vector<BlobBase*>& bottom,
      const vector<BlobBase*>& top) {
  int new_tops_num = indices_to_forward_.size();
...
template <typename Dtype, typename Mtype>
void FilterLayer<Dtype,Mtype>::Backward_cpu(const vector<BlobBase*>& top,
